...
- **Comment insertion:** `ContentView.swift:1090-1165` — `computeCommentInsertion`; empty selection mid-line → inline `<!--  -->`; empty selection on structural line (`#`/`-*/+`/`>`/`\d+\.`) follows cursor-position rule (marker zone → end-of-line; past marker → inline at cursor); selection without newline → inline after selection; selection with newline → block-above with newline; comment is always bare `<!--  -->` with no surrounding whitespace; cursor lands inside spaces at offset 5; posts `mtdCommentAdded`.
